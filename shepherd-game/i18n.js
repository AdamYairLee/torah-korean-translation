(() => {
  ...
  const readLanguage = () => {
    try { return localStorage.getItem('shepherdLanguage'); }
    catch { return null; }
  };
  const saveLanguage = value => {
    try { localStorage.setItem('shepherdLanguage', value); }
    catch (error) { console.warn('언어 설정 저장 실패:', error); }
  };
  ...
  const he = {
    ...
    '언어 선택':'בחירת שפה','한국어':'קוריאנית','키 소개':'מקשים ושליטה','설정으로 돌아가기':'חזרה להגדרות','두루마리 형태의 미니맵':'מפת גלילה',
  ...
  function dynamicTerm(value) {
    ...
    if (value === '+15 셰켈') return '15+ שקלים';
    return value;
  }
  ...
  const patterns = [
    ...
    [/^늑대 한 마리를 물리쳤습니다\. (.+)$/, (_all, reward) => `הבסת זאב אחד. ${dynamicTerm(reward)}`],
    [/^늑대 떼를 모두 물리쳤습니다\. 존중 \+10 · (.+)$/, (_all, reward) => `הבסת את כל להקת הזאבים. כבוד +10 · ${dynamicTerm(reward)}`],
    [/^(.+)을 물리쳤습니다\. 존중 \+5 · (.+)$/, (_all, enemy, reward) => `הבסת את ${defeatedEnemy(enemy)}. כבוד +5 · ${dynamicTerm(reward)}`],
    [/^남은 양이 5마리 미만이어서 셰켈 보상 없음$/, 'אין פרס שקלים משום שנותרו פחות מחמש כבשים'],
    [/^\+15 셰켈$/, '15+ שקלים'],
    [/^돌\s*([\d,]+)(\/25)?$/, 'אבנים $1$2'],[/^존중\s*([\d,]+)(\/100)?$/, 'כבוד $1$2'],[/^([\d,]+)\s*셰켈$/, '$1 שקלים'],
    [/^존중 \+([\d,]+) 상승 · 셰켈 \+15$/, 'כבוד +$1 · 15+ שקלים'],
    [/^(.+)에게 필살기 명중!$/, (_all, target) => `פגיעת המהלך המיוחד ${hitTarget(target)}!`],
    [/^(.+) 명중!$/, (_all, target) => `פגיעה ${hitTarget(target)}!`],
    [/^(.+)을 주웠습니다\. \(([\d,]+)\/25\)$/, (_all, stone, count) => `אספת ${dynamicTerm(stone)}. (${count}/25)`]
  ];
