...
function openKoreanGame() {
  const gameUrl = `./game/index.html?lang=${UI_LANG === 'en' ? 'en' : 'he'}`;
  trackEvent('open_korean_letter_game', { language: UI_LANG, link_url: gameUrl });
...
function renderJourneySection() {
  return `<section class="journey-section">
    ...
    <div class="journey-grid">
      ...
      <button type="button" class="journey-card" onclick="openKoreanGame()"><h3>${t('supportTitle')}</h3><p>${t('supportText')}</p><span>${t('gameAction')}</span></button>
    </div>
  </section>`;
}
